...
func handleFavorites(count int) error {
	...
	for i, v := range rndHServiceIds {
		if i%logPerLoop == 0 {
			logger.Debug("Generating favorite", logger.Args("i", i, "userId", userId, "hservice id", v))
		}

		_, err = d.Queries.CreateFavorite(ctx, db.CreateFavoriteParams{
			UserID:     userId,
			HserviceID: v,
		})
